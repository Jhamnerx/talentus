<x-form.modal.card title="{{ $compra ? 'Pagos de ' . ($compra->serie_correlativo ?? 'C-' . $compra->id) : 'Pagos' }}"
    ...
    @if ($compra)
        ...
        <div class="mb-4 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="flex justify-between items-center text-sm">
                <div>
                    <span class="font-medium text-gray-700 dark:text-gray-300">Proveedor:</span>
                    <span
                        class="text-gray-900 dark:text-gray-100 ml-2">{{ $compra->proveedor->razon_social ?? 'N/A' }}</span>
                </div>
                <div>
                    <span class="font-medium text-gray-700 dark:text-gray-300">Total:</span>
                    <span class="text-gray-900 dark:text-gray-100 ml-2 font-bold">{{ $compra->divisa }}
            ...
            <table class="min-w-full">
                <thead>
                    <tr
                        ...
                        <th class="px-2 py-2 text-left w-40">Desde (Origen)</th>
                ...
                <tbody class="text-sm">
                    ...
                    @foreach ($existing_payments as $payment)
                        <tr
                            ...
                            <td class="px-2 py-2 text-center">
                                <span
                                    class="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">
                                    ✓ Pagado
                                </span>
                            </td>
                            <td class="px-2 py-2 text-center">
                    ...
                    @if ($showNewPaymentRow)
                        <tr class="bg-red-50 dark:bg-red-900/20 border-b-2 border-red-200 dark:border-red-700">
                            ...
                            <td class="px-2 py-2">
                                <div class="w-40">
                                    @if ($this->paymentDestinations && $this->paymentDestinations->isNotEmpty())
                                        <x-form.select wire:model.defer="payment_destination_id" :options="$this->paymentDestinations->toArray()"
                                            option-label="description" option-value="id" placeholder="Desde (Origen)" />
                                    @else
                                        <span class="text-gray-500 text-xs">Sin orígenes</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-2 py-2">
                                <div class="w-28">
                                    <x-form.currency wire:model.defer="monto" placeholder="0.00" :prefix="$compra->divisa ?? 'S/'" />
                                </div>
                            </td>
                            <td class="px-2 py-2 text-center">
                                <span
                ...
                <tfoot class="bg-gray-50 dark:bg-gray-900/30 border-t-2 border-gray-200 dark:border-gray-700">
                    <tr class="text-sm">
                        ...
                        <td class="px-2 py-2 text-right font-bold text-red-600 dark:text-red-400">
                            {{ $compra->divisa }} {{ number_format($compra->total - $total_pendiente, 2) }}
                        </td>
                        <td colspan="2"></td>
                    </tr>
                    <tr class="text-sm">
                        ...
                        <td class="px-2 py-2 text-right font-bold text-amber-600 dark:text-amber-400">
                            {{ $compra->divisa }} {{ number_format($total_pendiente, 2) }}
                        </td>
                        <td colspan="2"></td>
                    </tr>
                </tfoot>
        ...
        @if (!$showNewPaymentRow && $total_pendiente > 0)
            <div class="mt-3 px-3">
                <x-form.button negative icon="plus" label="Agregar Pago" wire:click="showNewPayment" full />
            </div>
        @endif
    @endif
