    ...
    >
        @if ($messages->isNotEmpty())
            ...
            <div class="mb-4 flex justify-center">
                ...
                >
                    <svg wire:loading wire:target="loadMore" class="h-3.5 w-3.5 animate-spin" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8V0C5.4 0 0 5.4 0 12h4z"></path>
                    </svg>
                    <svg wire:loading.remove wire:target="loadMore" class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
        ...
        @forelse ($messages as $m)
            ...
            @if ($showSeparator && $m->created_at)
                <div class="my-4 flex justify-center" wire:key="sep-{{ $msgDate }}">
                    <span class="rounded-lg bg-white/85 px-3 py-1 text-[11px] font-medium uppercase tracking-wide text-gray-500 shadow-sm ring-1 ring-black/5 backdrop-blur dark:bg-gray-800/85 dark:text-gray-300 dark:ring-white/5">
            ...
            >
                ...
                >
                    ...
                    @if ($hasMedia && $mediaUrl)
                        ...
                        @elseif ($type === 'audio')
                            <audio controls preload="none" src="{{ $mediaUrl }}" class="mb-1 w-56 max-w-full"></audio>
                        @elseif ($type === 'video')
                        ...
                        @else
                            ...
                            >
                                <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-500/15 text-emerald-600 dark:text-emerald-300">
                                ...
                                <span class="min-w-0 flex-1">
                                    <span class="block truncate text-[13px] font-medium">{{ $m->file_name ?? 'Documento' }}</span>
                                    <span class="block text-[11px] text-gray-500 dark:text-gray-400">{{ strtoupper($m->mime_type ? pathinfo($m->file_name ?? '', PATHINFO_EXTENSION) : 'Archivo') ?: 'Archivo' }}</span>
                                </span>
                            </a>
                        @endif
                    @endif
                    ...
                    <div @class([
                        ...
                        @unless ($isContact)
                            @if ($status === 'pending')
                                <svg class="h-3 w-3 animate-pulse" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
                            ...
                            @elseif ($status === 'delivered')
                                <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m1.5 12.5 4 4L15 6m1.5 10.5L22 6" />
                                </svg>
                            @elseif ($status === 'read')
                                <svg class="h-3.5 w-3.5 text-sky-500 dark:text-sky-400" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m1.5 12.5 4 4L15 6m1.5 10.5L22 6" />
                                </svg>
                            @elseif ($status === 'failed')
                                <svg class="h-3.5 w-3.5 text-rose-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m0 3.75h.008M10.36 3.6 1.95 18a1.5 1.5 0 0 0 1.29 2.25h16.52A1.5 1.5 0 0 0 21.05 18L12.64 3.6a1.5 1.5 0 0 0-2.28 0Z" />
                                </svg>
                            @endif
                        @endunless
                    </div>
        ...
        @empty
            <div class="flex h-full flex-col items-center justify-center text-center">
                <div class="mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-white/70 shadow-sm ring-1 ring-black/5 dark:bg-gray-800/70 dark:ring-white/5">
                ...
                <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Envía el primer mensaje para iniciar la conversación.</p>
            </div>
        @endforelse
    </div>
