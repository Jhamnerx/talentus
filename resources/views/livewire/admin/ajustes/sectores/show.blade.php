<div class="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-5">
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Sectores de Vehículos ✨</h1>
            <span class="text-gray-500 dark:text-gray-400 text-sm">Define los sectores o regulaciones que se asignan a cada vehículo (SUTRAN, MINA, PERSONAL, etc.)</span>
        </div>
        <div class="flex items-center gap-2">
            <x-form.input wire:model.live="search" placeholder="Buscar sector..." icon="magnifying-glass" />
            <button wire:click="openModalSave" class="btn bg-indigo-500 hover:bg-indigo-600 text-white inline-flex items-center gap-2">
                ...
                <span class="hidden xs:block">Añadir Sector</span>
            </button>
        </div>
    </div>
        ...
        <table class="table-auto w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="text-sm divide-y divide-gray-200 dark:divide-gray-700">
                @forelse ($sectores as $sector)
                    <tr wire:key="sector-{{ $sector->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-900/30">
                        <td class="px-4 py-3 text-gray-400 dark:text-gray-500 text-xs w-12">{{ $sector->orden }}</td>
                        <td class="px-4 py-3">
                            <span class="font-semibold text-gray-800 dark:text-gray-100">{{ $sector->nombre }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3 w-px">
                            <div class="flex items-center justify-center gap-1">
                                <button wire:click="openModalEdit({{ $sector->id }})"
                                ...
                                <button wire:click="openModalDelete({{ $sector->id }})"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-10 text-center text-gray-400 dark:text-gray-500">
                            No hay sectores configurados aún
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="mt-6">{{ $sectores->links() }}</div>
</div>
