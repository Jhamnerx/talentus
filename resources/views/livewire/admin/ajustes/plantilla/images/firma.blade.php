    ...
    <div class="bg-white dark:bg-gray-800 rounded shadow-lg overflow-auto w-full max-h-full"
        ...
        <div class="px-5 py-3 border-b border-slate-200 dark:border-gray-700">
            <div class="flex justify-between items-center">
                <div class="font-semibold text-slate-800 dark:text-gray-100 uppercase">IMAGEN FIRMA</div>
            </div>
        </div>
        <!-- content -->
        @if ($plantilla->img_firma)
            <div class="px-4 py-5 bg-white dark:bg-gray-800 sm:p-6">
                <img src="{{ asset('storage/' . $plantilla->img_firma) }}" alt="">
            </div>
        @else
            <div class="px-4 py-5 bg-white dark:bg-gray-800 sm:p-6" x-data="{ file: @entangle('img_firma').live, files: null }">
                @if ($img_firma)
                    <div class="px-5 py-2 border-b ">
                        <div class="flex justify-between items-center">
                            <label class="block text-sm font-medium text-gray-700">
                            </label>
                            <button wire:click="$set('img_firma', null)" @click="files = null" type="button"
                ...
                <div class="mt-1 relative flex justify-center px-6 pt-5 pb-6 border-2 cursor-pointer border-gray-300 dark:border-gray-600 border-dashed rounded-md"
                    ...
                    <input wire:model.live="img_firma" type="file"
                    ...
                    <div class="space-y-1 text-center">
                        @if ($img_firma)
                            <img src="{{ $img_firma->temporaryUrl() }}">
                        @endif
        ...
        <div class="px-5 py-4 border-t border-slate-200 dark:border-gray-700">
            <div class="flex flex-wrap justify-end space-x-2">
                @can('admin.settings.plantilla.images.edit')
                    ...
                    @if ($plantilla->img_firma)
                        <button wire:click="openModalDelete('Imagen Firma', 'img_firma')" type="button"
                            class="mr-2 btn-sm bg-red-500 hover:bg-red-600 text-white">
                            Eliminar
                        </button>
                    @else
