<x-form.modal.card title="{{ $bank_account_id ? 'Editar Cuenta Bancaria' : 'Nueva Cuenta Bancaria' }}"
    wire:model="showModal" blur width="2xl">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <x-form.select label="Moneda" wire:model="currency_type_id">
            @foreach ($currencies as $currency)
                <x-select.option label="{{ $currency->name }}" value="{{ $currency->id }}" />
            @endforeach
        </x-form.select>
