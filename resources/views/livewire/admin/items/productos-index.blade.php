                            ...
                            <tr wire:key='product-{{ $producto->id }}'>
                                ...
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    @if ($producto->es_servicio_cobro)
                                        <span
                                            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-indigo-600 text-white dark:bg-indigo-500 shadow-sm">
                                ...
                                @can('cambiar.estado-producto')
                                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                        <x-form.toggle id="toggle-{{ $producto->id }}" name="toggle-{{ $producto->id }}"
                                            md :checked="(bool) $producto->estado" x-on:change="$wire.toggleStatus({{ $producto->id }})" />
                                    </td>
                                @endcan
                                @if (auth()->user()->can('editar-producto') || auth()->user()->can('eliminar-producto'))
                                    <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @can('eliminar-producto')
                                                @if ($producto->es_servicio_cobro)
                                                    <span title="No se puede eliminar: es el servicio de cobros"
                                                        class="text-slate-300 dark:text-gray-600 cursor-not-allowed">
                                                        <svg class="w-8 h-8 fill-current" viewBox="0 0 32 32">
                                                            <path
