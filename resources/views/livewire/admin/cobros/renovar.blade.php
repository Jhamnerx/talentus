<x-form.modal.card title="Renovar Período" wire:model="modalOpen" blur width="2xl">
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <x-form.datetime.picker wire:model="fechaFin" label="Fecha Fin" parse-format="YYYY-MM-DD"
                display-format="DD/MM/YYYY" without-time />
            <x-form.select wire:model="divisa" label="Divisa" :options="[['name' => 'Soles (PEN)', 'id' => 'PEN'], ['name' => 'Dólares (USD)', 'id' => 'USD']]" option-label="name" option-value="id"
                :clearable="false" />
        </div>

        <div class="grid grid-cols-2 gap-4">
            <x-form.input type="number" wire:model="monto" label="Monto" min="0" step="0.01" />
            <x-form.input type="number" wire:model="descuento" label="Descuento" min="0" step="0.01" />
        </div>

        <x-form.textarea wire:model="observaciones" label="Observaciones (opcional)" rows="2" />

        <div class="flex items-center gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
            <x-form.checkbox wire:model="cobrarAhora" id="cobrar-ahora-check" />
    ...
    <x-slot name="footer">
        <div class="flex justify-end gap-2">
            ...
            <x-form.button primary label="Renovar Período" wire:click="renovar" wire:loading.attr="disabled"
                spinner="renovar" />
        </div>
    </x-slot>
</x-form.modal.card>
