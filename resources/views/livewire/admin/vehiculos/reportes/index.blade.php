    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <button wire:click="$set('estado_filter', '{{ \App\Models\Reportes::ESTADO_ABIERTA }}')"
            ...
            <div class="flex items-center justify-between">
                <span
                    class="text-xs font-semibold uppercase text-red-600 dark:text-red-400 tracking-wide">Abiertas</span>
        ...
        <button wire:click="$set('estado_filter', '{{ \App\Models\Reportes::ESTADO_EN_ATENCION }}')"
            ...
            <div class="flex items-center justify-between">
                <span class="text-xs font-semibold uppercase text-blue-600 dark:text-blue-400 tracking-wide">En
                    atención</span>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-700 p-4">
            <div class="flex items-center justify-between">
                <span
                    class="text-xs font-semibold uppercase text-emerald-600 dark:text-emerald-400 tracking-wide">Cerradas
                    hoy</span>
        ...
        <button wire:click="$set('origen_filter', 'auto')"
            ...
            <div class="flex items-center justify-between">
                <span class="text-xs font-semibold uppercase text-violet-600 dark:text-violet-400 tracking-wide">Auto
                    hoy</span>
                ...
                <div class="relative">
                    <button @click.prevent="open = !open"
                        ...
                        <span class="text-gray-500 dark:text-gray-400">
                            @if ($from)
                                ...
                                {{ \Carbon\Carbon::parse($to)->format('d/m') }}
                            @else
                                Todos
                            @endif
                        </span>
                    ...
                    <div x-show="open" @click.outside="open = false" x-cloak
                        ...
                        <button wire:click="filter(1)" @click="open=false"
                            class="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300">Hoy</button>
                        <button wire:click="filter(7)" @click="open=false"
                            class="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300">Últimos
                            7 días</button>
                        <button wire:click="filter(30)" @click="open=false"
                            class="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300">Últimos
                            30 días</button>
                        <button wire:click="filter(0)" @click="open=false"
                            class="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300">Todos</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="divide-y divide-gray-100 dark:divide-gray-700/60">
            @forelse ($reportes as $reporte)
                ...
                <div wire:key="reporte-{{ $reporte->id }}"
                    class="p-4 hover:bg-gray-50/70 dark:hover:bg-gray-900/20 transition-colors">
                    ...
                    <div class="flex flex-wrap items-start gap-3">
                        ...
                        <div class="mt-0.5 shrink-0">
                            ...
                            @elseif($reporte->estado == \App\Models\Reportes::ESTADO_EN_ATENCION)
                                <span
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400">
                                    <span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span>En atención
                                </span>
                            @else
                                <span
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400">
                        ...
                        @if ($reporte->origen === 'auto')
                            <span
                                class="mt-0.5 inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-400 uppercase tracking-wide">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex flex-wrap items-baseline gap-2">
                                ...
                                @if ($veh?->cliente)
                                    ...
                                    @else
                                        <span
                                            class="text-sm text-gray-600 dark:text-gray-300">{{ $veh->cliente->razon_social }}</span>
                                    @endif
                                @endif
                            </div>
                            ...
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
                                @if ($reporte->horas_sin_transmision)
                                    <span
                                        class="inline-flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400 font-medium">
                                ...
                                @if ($reporte->user)
                                    <span class="text-xs text-gray-400 dark:text-gray-500">por
                                        {{ $reporte->user->name }}</span>
                                @endif
                                ...
                                @if ($reporte->atendidoPor)
                                    <span class="text-xs text-blue-500 dark:text-blue-400">
                                        ...
                                        @if ($reporte->atendido_at)
                                            · {{ $reporte->atendido_at->diffForHumans() }}
                                        @endif
                                    </span>
                                @endif
                        ...
                        <div class="flex items-center gap-2 shrink-0 mt-0.5">
                            @if ($reporte->estado == \App\Models\Reportes::ESTADO_ABIERTA)
                                <x-form.button wire:click="atender({{ $reporte->id }})"
                                    wire:confirm="¿Tomar esta alerta?" info xs icon="hand-raised" label="Atender" />
                            @elseif($reporte->estado == \App\Models\Reportes::ESTADO_EN_ATENCION)
                                <x-form.button wire:click="cerrar({{ $reporte->id }})"
                                    wire:confirm="¿Marcar como resuelta?" positive xs icon="check"
                                    label="Cerrar" />
                            @else
                                <x-form.button wire:click="reabrir({{ $reporte->id }})" flat secondary xs
                                    icon="arrow-uturn-left" label="Reabrir" />
                            @endif
                            ...
                            <div class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.outside="open = false" x-cloak
                                    ...
                                    <button wire:click="openModalRecordatorio({{ $reporte->id }})"
                                        @click="open=false"
                                        class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                    ...
                    @if ($detalleAbierto === $reporte->id)
                        <div class="mt-3 ml-0 md:ml-6 bg-gray-50 dark:bg-gray-900/30 rounded-lg p-3 space-y-2">
                            ...
                            <div class="flex items-end gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                                <textarea wire:model.live="nuevoDetalle" rows="2"
                                ...
                                <x-form.button wire:click="guardarDetalle({{ $reporte->id }})" primary xs
                                    icon="paper-airplane" label="Guardar" />
                            </div>
                            @error('nuevoDetalle')
