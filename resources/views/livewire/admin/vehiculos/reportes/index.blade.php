<div class="px-4 sm:px-6 lg:px-8 py-8 w-full mx-auto">
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-5">
        ...
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">REPORTE TRANSMISIÓN</h1>
        </div>
        ...
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            ...
            <!-- GPS Status button -->
            <button wire:click.prevent="openModalEstadoTransmision()"
                class="btn bg-emerald-500 hover:bg-emerald-600 text-white">
                <svg class="w-4 h-4 fill-current opacity-50 shrink-0" viewBox="0 0 16 16"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M8 0C3.6 0 0 3.1 0 7c0 3.9 3.6 9 8 9s8-5.1 8-9c0-3.9-3.6-7-8-7zm0 13c-2.7-2.7-5-5.9-5-7 0-2.8 2.2-5 5-5s5 2.2 5 5c0 1.1-2.3 4.3-5 7z" />
                    <circle cx="8" cy="7" r="2" />
                </svg>
                <span class="hidden xs:block ml-2">Estado GPS</span>
            </button>
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-5">
        ...
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            ...
            <div class="table-items-action hidden">
                <div class="flex items-center">
                    <div class="hidden xl:block text-sm italic mr-2 whitespace-nowrap text-gray-500 dark:text-gray-400">
                        <span class="table-items-count"></span> Items Seleccionados</div>
                    <button
            ...
            <div class="relative float-right" x-data="{ open: false, selected: 4 }">
                <button wire:ignore
                    class="btn justify-between min-w-44 bg-white dark:bg-gray-800 border-slate-200 dark:border-gray-700/60 hover:border-slate-300 dark:hover:border-gray-600 text-slate-500 dark:text-gray-400 hover:text-slate-600 dark:hover:text-gray-300"
                    aria-label="Select date range" aria-haspopup="true" @click.prevent="open = !open"
                    :aria-expanded="open">
                    <span class="flex items-center">
                        <svg class="w-4 h-4 fill-current text-slate-500 dark:text-gray-400 shrink-0 mr-2"
                            viewBox="0 0 16 16">
                            <path
                    ...
                    <svg class="shrink-0 ml-1 fill-current text-slate-400 dark:text-gray-500" width="11"
                        height="7" viewBox="0 0 11 7">
                        <path d="M5.4 6.8L0 1.4 1.4 0l4 4 4-4 1.4 1.4z" />
                    </svg>
                </button>
                <div class="z-10 absolute top-full right-0 w-full bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700/60 py-1.5 rounded shadow-lg overflow-hidden mt-1"
                    ...
                    <div class="font-medium text-sm text-slate-600 dark:text-gray-300" x-ref="options">
                        <button wire:click="filter(1)" tabindex="0"
                            class="flex items-center w-full hover:bg-slate-50 dark:hover:bg-gray-700/50 py-1 px-3 cursor-pointer"
    ...
    <div class="bg-white dark:bg-gray-800 shadow-xs rounded-xl mb-8">
        <header class="px-5 py-4">
            <h2 class="font-semibold text-gray-800 dark:text-gray-100">Reportes <span
                    class="text-gray-400 dark:text-gray-500 font-medium">{{ $reportes->total() }}</span>
            </h2>
        </header>
        <div>
                            ...
                            <tr wire:key='reporte-{{ $reporte->id }}'>
                                ...
                                <td class="px-2 first:pl-5 last:pr-5 py-3">
                                    @if ($reporte->vehiculos->cliente)
                                        @if (count($reporte->vehiculos->cliente->contactos) > 0)
                                            <div wire:click="openModalContactos({{ $reporte->vehiculos->cliente->id }})"
                                                class="font-medium text-slate-800 dark:text-gray-300 cursor-pointer hover:shadow-inner hover:text-blue-600 dark:hover:text-blue-400 hover:font-semibold">
                                    ...
                                    <p class="text-xs">
                                        ...
                                        @else
                                            <div class="font-sm text-slate-900 dark:text-gray-300">
                                                <p class="text-xs">
                                                    {{ $reporte->vehiculos->cliente->razon_social }}
                                                </p>
                                ...
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    ...
                                    @if ($reporte->estado == 1)
                                        <div
                                            class="text-xs inline-flex font-medium bg-rose-100 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400 rounded-full text-center px-2.5 py-1">
                                            Por Consultar
                                        </div>
                                    @elseif($reporte->estado == 2)
                                        <div
                                            class="text-xs inline-flex font-medium bg-sky-100 dark:bg-sky-500/20 text-sky-600 dark:text-sky-400 rounded-full text-center px-2.5 py-1">
                                            En Espera
                                        </div>
                                    @else
                                        <div
                                            class="text-xs inline-flex font-medium bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-full text-center px-2.5 py-1">
                                            Solucionado</div>
                                    @endif

                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div class="font-medium text-slate-800 dark:text-gray-300">
                                        {{ $reporte->fecha_t->format('d-m-Y') }}</div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                    <div class="font-medium text-slate-800 dark:text-gray-300">{{ $reporte->hora_t }}
                                    </div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                ...
                                <td class="px-2 first:pl-5 last:pr-5 py-3 ">
                                    <div class="font-medium text-slate-800 dark:text-gray-300">{{ $reporte->detalle }}
                                    </div>
                                </td>
                                <td class="px-2 first:pl-5 last:pr-5 py-3">
                                    <div class="font-medium text-slate-800 dark:text-gray-300">
                                    ...
                                    <div class="relative inline-flex" x-data="{ open: false }">
                                        <div class="relative inline-block h-full text-left">
                                            <button
                                                ...
                                                :class="{ 'bg-slate-100 dark:bg-gray-700/50 text-slate-500 dark:text-gray-400': open }"
                                            ...
                                            <div class="origin-top-right  z-10 absolute transform  -translate-x-3/4  top-full left-0 min-w-36 bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700/60 py-1.5 rounded shadow-lg overflow-hidden mt-1  ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-gray-700/60 focus:outline-none"
                                                ...
                                                <ul>
                                                    <li>
                                                        <a href="javascript: void(0)"
                                                            ...
                                                            class="text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 group flex items-center px-4 py-2 text-sm font-normal"
                                                    ...
                                                    <li>
                                                        ...
                                                        <a href="javascript: void(0)"
                                                            ...
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                                ...
                                                                class="w-5 h-5 mr-3 text-gray-400 dark:text-gray-500 group-hover:text-blue-500 dark:group-hover:text-blue-400">
                                                    ...
                                                    <li>
                                                        ...
                                                        <a href="javascript: void(0)"
                                                            ...
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                                viewBox="0 0 24 24" stroke="currentColor"
                                                                class="h-5 w-5 mr-3 text-gray-400 dark:text-gray-500 group-hover:text-red-500 dark:group-hover:text-red-400">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2"
                                                    ...
                                                    <li>
                                                        <a href="javascript: void(0)"
                                                            ...
                                                            role="menuitem" tabindex="-1">
                                                            <svg class="h-5 w-5  mr-3 text-gray-400 dark:text-gray-500 group-hover:text-emerald-500 dark:group-hover:text-emerald-400"
                                                                xmlns="http://www.w3.org/2000/svg"
                                                                viewBox="0 0 24 24">
                                                    ...
                                                    <li>
                                                        <a href="{{ route('admin.vehiculos.reportes.show', $reporte) }}"
                                                            ...
                                                            role="menuitem" tabindex="-1"><svg
                                                                ...
                                                                viewBox="0 0 24 24" stroke="currentColor"
                                                                class="h-5 w-5  mr-3 text-gray-400 dark:text-gray-500 group-hover:text-violet-500 dark:group-hover:text-violet-400">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2"
