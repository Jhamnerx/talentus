<div class="bg-white shadow-lg rounded-sm border border-slate-200 mb-8">
    <header class="px-5 py-4">
        <h2 class="font-semibold text-slate-800">Ventas <span
                class="text-slate-400 font-medium">{{ $notas->total() }}</span></h2>
    </header>
    <div>
                                ...
                                @if ($nota->tipo_comprobante_id != '02')
                                    <div class="space-x-1">
                                        ...
                                        @switch($nota->fe_estado)
                                            {{-- OBTENER CDR --}}
                                            @case('0')
                                                <x-form.button.circle flat wire:click.prevent="getCdr({{ $nota->id }})"
                                                    teal icon="refresh" spinner='getCdr({{ $nota->id }})' />
                                            @break
