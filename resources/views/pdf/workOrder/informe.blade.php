...
@if ($woBg)
    ...
    <body
        style="background-image: url('data:image/png;base64,{{ $woBg }}'); background-size: 100% 100%; background-repeat: no-repeat; background-position: top left;">
    @else

        <body>
@endif

{{-- Código WO posicionado sobre el encabezado de la imagen --}}
@if ($woBg)
    <div
...
<div id="wo-content">
        ...
        <div class="info-grid">
            ...
            @if ($workOrder->fecha_inicio)
                <div class="info-row">
                    ...
                    <div class="info-value">{{ $workOrder->fecha_inicio->format('d/m/Y H:i') }}</div>
                </div>
            @endif
            @if ($workOrder->fecha_finalizacion)
                <div class="info-row">
                    <div class="info-label">Fecha Finalización:</div>
                    <div class="info-value">{{ $workOrder->fecha_finalizacion->format('d/m/Y H:i') }}</div>
                </div>
            @endif
        </div>
    </div>
        ...
        <div class="info-section" style="border-left: 4px solid #2563eb; background: #f0f4ff;">
            ...
            <div class="info-grid">
                <div class="info-row">
                    ...
                    <div class="info-value" style="font-weight: bold; font-family: monospace;">{{ $mt->numero }}
                    </div>
                </div>
                <div class="info-row">
                    <div class="info-label">Fecha Programada:</div>
                ...
                @if ($mt->nota)
                    <div class="info-row">
                        ...
                        <div class="info-value" style="font-style: italic;">{{ $mt->nota }}</div>
                    </div>
                @endif
            </div>
        </div>
    ...
    @if ($workOrder->observaciones)
        <div class="info-section">
            <h2>Observaciones</h2>
            <p style="margin: 0; font-size: 11px;">{{ $workOrder->observaciones }}</p>
        </div>
    @endif
