            ...
            <div class="bg-white rounded shadow-lg overflow-auto max-w-lg w-full max-h-full"
                ...
                <div class="px-5 py-4">
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <input id="to" class="form-input w-full px-2 py-1 disabled:bg-slate-100" wire:model.live="to"
                                type="email" required disabled />
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1" for="asunto">Asunto: <span
                                    class="text-rose-500">*</span></label>
                            <input id="asunto" class="form-input w-full px-2 py-1" wire:model.live="asunto" type="text" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1" for="body">Mensaje:
                            </label>
                            <textarea id="body" class="form-textarea w-full px-2 py-1" wire:model.live="body" rows="5"
                                required></textarea>
                        </div>
                ...
                <div class="px-5 py-4 border-t border-slate-200" x-data="{ estado: <?php if ((object) ('disabled') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('disabled'->value()); ?>')<?php echo e('disabled'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('disabled'); ?>')<?php endif; ?>.live }">
                    <div class="flex flex-wrap justify-end space-x-2">
                        ...
                        <button wire:loading.attr="disabled" wire:target="sendActa" x-bind:disabled="estado"
                            class="btn-sm bg-indigo-500 hover:bg-indigo-600 disabled:bg-indigo-300 text-white"
                            wire:click="sendActa">Enviar</button>
                    </div>
                </div>
            </div>
        </div>
...
<?php /**PATH C:\laragon2\www\talentus\resources\views/livewire/admin/certificados/actas/send.blade.php ENDPATH**/ ?>